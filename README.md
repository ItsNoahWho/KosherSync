...
is Luau, and `ksync` is a script. `npm start` is the whole install.
...
On macOS `npm start` builds `dist/KosherSync.app` and opens that, because
...
open dist/KosherSync.app --args --remote-debugging-port=9333
```

Then open a place in Studio. The plugin connects to the daemon on port 7900, and
`ksync` starts answering.

```bash
npm link          # puts `ksync` on your PATH
ksync status
```

`npm link` is what makes every `ksync ...` in this README work; without it the
long form is `node cli/ksync.js status`. `npm unlink -g koshersync` undoes it.
...
`ksync help` lists everything; `ksync help <command>` explains one. A tour of the
parts worth knowing:

```bash
ksync ls Workspace                        # navigate
ksync get Workspace/Baseplate Size
ksync tree StarterGui --depth 3
ksync query 'StarterGui/**/TextButton'    # ** matches any depth

ksync set Workspace/Part Position 0,6,0   # write
ksync set Workspace/Part Color '#3b82f6'
ksync set Workspace/Part Material Neon
ksync new Part --parent Workspace --name Boss
ksync copy Workspace/Boss && ksync paste ReplicatedStorage
```
...
ksync photo --subject Workspace/Boss --out boss.png    # frames the camera on it
ksync photo --subject Workspace/Boss --margin 2        # more air around it
```
...
`ksync test` runs a Luau file inside a real playtest: live DataModel, spawned
player, real replication.

```bash
ksync test checks/spawn.luau
ksync test checks/hud.luau --context client
ksync test checks/net.luau --mode multiplayer --players 2
```
...
```bash
ksync playtest              # start
ksync run 'return #game.Players:GetPlayers()'
ksync run --script probe.luau --context client
ksync stop
```
...
| 6 | a test failed |
| 7 | the setup is broken — `ksync doctor` says what |
...
```
<project>/.koshersync/commands/    →    ~/.koshersync/commands/    →    commands/
```
...
```bash
ksync agents                      # print it
ksync agents --install            # add or refresh the section in ./AGENTS.md
ksync agents --install ~/MyGame   # or somewhere else
```
...
Projects KosherSync creates get both automatically. Installing is idempotent and
...
A project decides which commands exist in it. Put a `.koshersync/config.json`
at its root:
...
`ksync help`, from `ksync commands`, and from the generated AGENTS.md. An agent
...
files, and their daemons watch the filesystem. KosherSync's control commands
had no such path.

So the daemon also listens on a unix socket at `~/.koshersync/daemon.sock`, and
`ksync` prefers it. A socket is a file, gated by filesystem permissions rather
than the network stack, so a network sandbox does not touch it. Nothing to
configure — `ksync doctor` says which door answered.
